...
class LunarOracle(object):
    ...
    def predict2(self, s):
            s = s.squeeze() # there are no batches here anyway
            angle_targ = s[0]*0 + s[2]*1.0 # angle should point towards center (s[0] is horizontal coordinate, s[2] hor speed)
            if angle_targ > 0.4: angle_targ = 0.4 # more than 0.4 radians (22 degrees) is bad
            if angle_targ < -0.4: angle_targ = -0.4
            hover_targ = 0.*np.abs(s[0]) # target y should be proporional to horizontal offset
