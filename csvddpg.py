...
def train(sess, env, args, actor, critic, actor_noise):
    ...
    for i in range(int(args['max_episodes'])):
        ...
        for j in range(int(args['max_episode_len'])):
            ...
            if terminal:
                break
        print(csvformatter.format(args['env'], args['random_seed'], 'DDPG', 0, i, int(ep_reward)))

def main(args):
    global csvformatter
    csvformatter = '{:<25}{:<11}{:<9}{:<8}{:<8}{:<8}' 

    if args['header']:
        print(csvformatter.format('Environment','Seed','Control','Error','Episode','Reward'))
...
if __name__ == '__main__':
    ...
    parser.set_defaults(use_gym_monitor=False) # Original
    parser.set_defaults(use_gym_monitor=False) # Original
